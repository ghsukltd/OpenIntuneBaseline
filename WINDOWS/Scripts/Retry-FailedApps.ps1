# Script orignally from https://call4cloud.nl/wp-content/uploads/2022/07/grsremediation.txt
# https://call4cloud.nl/retry-failed-win32app-installation/
# Modified by GHS to be used as a one-time run script for Intune Management Extension to retry failed Win32 app installations.
## Reviewed 27/05/2025
## Template Revision 1.2

#---------- Script Variables ----------#
$ScriptName = "Intune_App_Failure_Remediation"
$ScriptVersion = "v1.0"
...
function Remove-FailedAppRegistryKeys {
    ...
    foreach ($path in $pathsToRemove) {
        if (Test-Path -Path $path) {
            ...
            Add-LogMessage -message "Removed registry key: $path"
        }
        else {
            Add-LogMessage -message "Registry key not found: $path"
        }
    }
}
...
foreach ($state in $failedStates) {
    ...
    Add-LogMessage -message "Installation of AppID: $($appId) failed for user $($userName) with error code: $($state.ErrorCode) - $($errorDescription)"
...
if ($failedStates.Count -gt 0) {
    Add-LogMessage -message "Detected $($failedStates.Count) failures." -warning
    Add-LogMessage -message "Restarting Intune Management Extension service..."
    Restart-Service -Name 'IntuneManagementExtension' -Force -PassThru
}

if ($failedStates.Count -eq 0) {
    Add-LogMessage -message "No failures detected."
} 
